...
from honeycomb_cell.render_stl_views import load_stl, project_triangle, rotate_point, _triangle_normal
...
def render_dimensioned_views(stl_path: str, output_path: str) -> None:
    ...
    p_left_out = (-62.5, 0.0, 100.0)
    p_right_out = (62.5, 0.0, 100.0)
    p_top_out = (0.0, 48.0, 100.0)
    p_bottom_out = (0.0, -48.0, 100.0)
    p_top_in = (0.0, 45.7, 100.0) # 48.0 - 2.3
    ...
    draw.text(((sf_left[0] + sf_right[0])/2 - 25, y_off_w - 18), "125.0 mm", fill=dim_color, font=font)

    # Height Dimension (96.0 mm)
    ...
    draw.text((x_off_h - 55, (sf_bottom[1] + sf_top[1])/2 - 8), "96.0 mm", fill=dim_color, font=font)
    ...
    # Length (200.0 mm)
    p_len_start = (0.0, 48.0, 0.0)
    p_len_end = (0.0, 48.0, 200.0)
    ...
    # End Fillet Radius
    draw.text((ss_start[0] - 80, ss_start[1] - 40), "Fillet R 0.8 mm", fill=dim_color, font=font)
    draw.line([(ss_start[0] - 5, ss_start[1] + 5), (ss_start[0] - 30, ss_start[1] - 30)], fill=dim_color, width=1)

    image.save(output_path)
    print(f"Dimensioned views saved successfully to: {output_path}")

# Run code directly if executed as main/script inside FreeCAD
stl_path = os.path.join(PROJECT_DIR, "honeycomb_cell", "output", "HoneycombCell.stl")
output_path = os.path.join(PROJECT_DIR, "honeycomb_cell", "output", "HoneycombCell_dimensioned.png")
render_dimensioned_views(stl_path, output_path)
