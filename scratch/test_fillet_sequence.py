...
cfg_test = HoneycombConfig(
    ...
    reinforcement_edge_fillet=0.0 # disable 3D longitudinal fillet, use 2D fillets instead!
)

print("Starting test_fillet_sequence...")
# 1. 2D profile
vertices_out = [
...
print("Extruded base solid successfully.")

# 2. Apply 45-degree end cuts to male joints (in place on the unified solid!)
# Wait, let's cut them with boxes.
# Since we cut them in place, we can rotate/translate the cutting boxes.
for face_index in (0, 5):
    alpha_deg, _ = get_face_angle_and_dist(face_index, cfg)
    face_mid = (vertices_out[face_index] + vertices_out[(face_index + 1) % 6]) * 0.5
    
    # We want a 45-degree slant tapering the tip.
    # Start cut box
...
print("Applied 45-degree end cuts successfully.")
