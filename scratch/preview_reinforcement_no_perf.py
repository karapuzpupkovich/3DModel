...
def write_binary_stl(shape, path: str, deflection: float = 0.5) -> None:
    ...
    with open(path, "wb") as handle:
        handle.write(b"HoneycombReinforcementPreview".ljust(80, b" "))
...
def main() -> None:
    cfg = replace(DEFAULT_CONFIG, cell_len=56.0)
    shape, report = create_honeycomb_cell_shape(cfg, enable_perforation=False, log=print)

    doc = FreeCAD.newDocument("HoneycombReinforcementPreview")
    obj = doc.addObject("Part::Feature", "HoneycombReinforcementPreview")
    obj.Shape = shape
    doc.recompute()

    output_dir = os.path.join(PROJECT_DIR, "honeycomb_cell", "output")
    os.makedirs(output_dir, exist_ok=True)
    stl_path = os.path.join(output_dir, "HoneycombReinforcementPreview.stl")
    write_binary_stl(shape, stl_path)

    print(f"Preview valid: {report.valid}")
    print(stl_path)
